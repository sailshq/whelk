...
module.exports = function whelk(optsOrMachineDef, omen){
  ...
  var machineDef;
  var opts;
  var MISC_OPTIONS = ['args', 'envVarNamespace', 'sails', 'useRawOutput'];
  ...
  // `args` is a special case, since it's tolerated by the machine spec.
  ...
  var wetMachine;
  try {
    wetMachine = Machine.build(machineDef, omen);
  } catch (err) {
    throw flaverr({
      name: 'ImplementationError',
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // Note that `await`, `.exec()`, `.then()`, `.log()`, etc should NOT be used anymore!
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  // console.log('->', util.inspect(yargs.argv,{depth:null}));
  // console.log('->', require('util').inspect(process.argv,{depth:null}));

  // TODO: Figure out a better way to solve this.  Or at least specifically understand common REPLs
  // (at least `node` and `sails console`)
  if (process.argv.length < 2) {
    throw flaverr({
      name: 'UsageError',
      message: 'Could not run shell script: `argv` is shorter than expected.  (Are you running this from the REPL?)'
    });
  }
