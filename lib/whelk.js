...
module.exports = function whelk(optsOrMachineDef, omen){
  ...
  if (opts.args && !machineDef.args) {
    ...
    throw flaverr({
      ...
      message: 'Invalid shell script: Definition and whelk-specific script options cannot BOTH contain an `args` declaration.'
    });
  }
  ...
  (function (proceed){
    ...
    deferred.exec(function(err, result){
      ...
      (function _doTeardownMaybe (proceed){
        ...
        if (result !== undefined) {
          if (opts.useRawOutput) {
            process.stdout.write(_.isString(result) ? result : util.inspect(result,{depth:5}));
          }
          else {
            console.log(
              _.isString(result) ?
