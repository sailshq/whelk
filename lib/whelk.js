...
module.exports = function whelk(optsOrMachineDef, omen){
  ...
  var omen;
  if (!omen && (process.env.NODE_ENV !== 'production' || process.env.DEBUG)) {
    omen = flaverr({}, new Error('whelk omen'), whelk);
  }
    ...
    switch (optKey) {
      ...
      case 'useRawOutput': (function (){
        if (!_.isBoolean(opts.useRawOutput)) {
          throw flaverr({
            name: 'ImplementationError',
            message: 'Invalid script: If specified, `useRawOutput` should be provided as either `true` or `false`.'
          });
        }
      })(); break;
